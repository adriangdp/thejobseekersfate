...
{#if isCardDisplay}
    <div class="relative w-full aspect-2/3 bg-accent-inner p-3 border-accent-outer border-1 bg-radial from-white/0 to-white/0 overflow-hidden">        
        <div class="relative mx-auto h-1/2 flex flex-col justify-end">
            <img src={status.icon} alt={`${status} icon status`} class="mx-auto mix-blend-color-dodge"/>
            <p class="text-lg text-center">{status.figureName}</p>
        </div>
        <div class="relative z-20">
            <p class="text-text-darker">{position}</p>
            <p>{company}</p>
            <p>{mode}</p>
            <p>{salary}</p>
